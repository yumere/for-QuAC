...
def read_coqa_examples(input_file, is_training, max_question_len=64):
    ...
    for entry in tqdm(input_data, desc="read"):
        ...
        for qa_idx, q in enumerate(entry["questions"]):
            qas_id = entry['id'] + '#' + str(q["turn_id"])

            question_text = q['input_text']

            # If you want to reflect previous question and answer
            '''
            if qa_idx > 0:
                question_text = '{}<q>{}<a>{}'.format(entry['questions'][qa_idx - 1]['input_text'],
                                                      entry['answers'][qa_idx - 1]['input_text'],
                                                      q['input_text'])
            '''
            ...
            if is_training:
                answer_text = entry['answers'][qa_idx]['input_text']
                span_text = entry['answers'][qa_idx]['span_text']
                ...
                if ans_choice == 3:
                    answer_offset = entry['answers'][qa_idx]['span_start'] + char_i
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    features = []
    for (example_index, example) in enumerate(tqdm(examples, desc="convert")):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            #     if is_training and not example.ans_choice == 3:
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if feature_null_score < score_null:
                ...
                null_class_logit = result.class_logits
...
def normalize_answer(s, len_preserved=False):
...
# Free text와 span을 비교해서 best index를 찾는다.
def free_text_to_span(free_text, full_text):
    ...
    # free text를 normalize하고 split한 단어 얻기
    free_ls = len_preserved_normalize_answer(free_text).split()
    # full text를 normalize하고 split해서 단어와 index값 얻기

...
from coqa_baselines.CoQA_eval import CoQAEvaluator
...
class OrderNet(nn.Module):
    def __init__(self, in_features, read_hidden_size, read_out_features, rnn_hidden_size, dropout, attn_dropout):
        super(OrderNet, self).__init__()

        self.rnn_hidden_size = rnn_hidden_size
        self.read = self.create_read_module(in_features, read_hidden_size, read_out_features, 1)

        self.encoder = nn.LSTMCell(read_out_features, rnn_hidden_size)
        self.encoder_attn = nn.MultiheadAttention(rnn_hidden_size, 1, attn_dropout,
                                                  kdim=read_out_features, vdim=read_out_features)

        self.decoder = nn.LSTMCell(read_out_features, rnn_hidden_size)
        self.decoder_attn = nn.MultiheadAttention(rnn_hidden_size, 1, attn_dropout,
                                                  kdim=read_out_features, vdim=read_out_features)

        self.proj = nn.Linear(rnn_hidden_size * 2, rnn_hidden_size, bias=False)
        self.dropout = nn.Dropout(dropout)

    def forward(self, inputs, mask=None, proc_step=5):
        """

        :param inputs: torch.FloatTensor batch_size x q_len x hidden_size
        :param mask: torch.LongTensor batch_size x q_len, meaning that which question is used for padding.
        :param proc_step: int, meaning that how many read module encode each questions,
        which results in the hidden states for the decoder
        """
        device = inputs.device
        batch_size, q_len, hidden_size = inputs.shape
        inputs = inputs.reshape(batch_size * q_len, -1)

        memory = self.read(self.dropout(inputs))
        memory = memory.reshape(batch_size, q_len, -1)
    ...
    def create_read_module(self, in_features, hidden_size, out_features, n_layer=1):
        in_layer = nn.Sequential(nn.BatchNorm1d(in_features), nn.Linear(in_features, hidden_size), GeLU())

        inner_layer = nn.Sequential(nn.BatchNorm1d(hidden_size), nn.Linear(hidden_size, hidden_size), GeLU())
        inner_layer = nn.Sequential(*[deepcopy(inner_layer) for i in range(n_layer)])

        out_layer = nn.Sequential(nn.BatchNorm1d(hidden_size), nn.Linear(hidden_size, out_features))
...
class CoQABaseline(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.order_net = OrderNet(config.hidden_size, config.hidden_size, config.hidden_size, 1024,
                                  config.hidden_dropout_prob, config.attention_probs_dropout_prob)

        # Unanswerable, yes, no, span
...
def evaluate(args, model, tokenizer, prefix=""):
        ...
        for i, example_index in enumerate(example_indices):
            ...
            if args.model_type in ['xlnet', 'xlm']:
                ...
                result = RawResult(unique_id=unique_id,
                                   ...
                                   class_logits=to_list(outputs[2][i]))
    ...
    if args.model_type in ['xlnet', 'xlm']:
        ...
        write_predictions_extended(examples, features, all_results, args.n_best_size,
                                   ...
                                   tokenizer, args.verbose_logging)
    else:
        pred, nbest_pred = write_predictions(examples, features, all_results, args.n_best_size,
                                             args.max_answer_length, args.do_lower_case, output_prediction_file,
                                             output_nbest_file, output_null_log_odds_file, args.verbose_logging,
                                             args.null_score_diff_threshold)
    evaluator = CoQAEvaluator(args.predict_file)
    performance = coqa_performance(pred, nbest_pred, evaluator)
    in_domain_f1 = performance['in_domain']['f1']
    out_domain_f1 = performance['out_domain']['f1']
    overall_f1 = performance['overall']['f1']

    return in_domain_f1, out_domain_f1, overall_f1
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
        ...
        examples = read_coqa_examples(input_file=input_file,
    ...
    all_ans_choice = torch.tensor([f.ans_choice for f in features], dtype=torch.long)
    ...
    if evaluate:
        ...
        dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                                all_start_positions, all_end_positions, all_ans_choice)
...
def main():
    ...
    parser.add_argument("--predict_file", default=None, type=str, required=True,
                        help="CoQA json for predictions. E.g., dev-v1.1.json or test-v1.1.json")

    # Other parameters
    parser.add_argument("--output_file", default=None, type=str, help="-i")
    ...
    parser.add_argument('--from_order_weights', type=str, default=None, help="pre-trained weights path")
    ...
    # TODO: This is not necessary code when initialize weights from sequence-model
    # TODO: make the model use another pre-trained model
    model = CoQABaseline.from_pretrained(args.model_name_or_path, config=config)
    logging.info("Load from bert pre-trained model")

    if args.from_order_weights:
        logging.info("Load from {}".format(args.from_order_weights))
        order_weights = torch.load(os.path.join(args.from_order_weights, "pytorch_model.bin"))
        model.initialize_from_order_weight(order_weights)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        checkpoints = sorted([(c, int(re.search(r"checkpoint-([\d]+)", c).groups()[0])) for c in checkpoints], key=lambda x: x[1])
        ...
        for checkpoint, global_step in checkpoints:
            # Reload the model
            model = CoQABaseline.from_pretrained(checkpoint)
            model.to(args.device)

            # Evaluate
            in_domain_f1, out_domain_f1, overall_f1 = evaluate(args, model, tokenizer, prefix=global_step)

            # result = dict((k + ('_{}'.format(global_step) if global_step else ''), v) for k, v in result.items())
            result = dict(in_domain_f1=in_domain_f1,out_domain_f1=out_domain_f1,overall_f1=overall_f1)
